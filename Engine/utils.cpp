...
// SetColors
void utils::SetColor(const float r, const float g, const float b, const float a)
{
	glColor4f(r, g, b, a);
}
...
void utils::SetRGBColor(int r, int g, int b)
{
	utils::SetColor(utils::GetRGBColor(r, g, b));
}
...
void utils::DrawEllipse( float centerX, float centerY, float radX, float radY, float lineWidth )
{
	if (radX > 0 && radY > 0 && lineWidth > 0)
	{
		...
		glBegin(GL_LINE_LOOP);
		{
			for (float angle = 0.0; angle < float(2.f * M_PI); angle += dAngle)
			{
...
Point2f utils::GetRandomPos(const Rectf& r, float border)
{
	return Point2f{ r.left + float(rand() % (int(r.width - (border * 2.f)))) + border, r.bottom + float(rand() % (int(r.height - (border * 2.f)))) + border };
}

Vector2f utils::GetRandomVector(const int range, bool negative)
{
	if (negative)
	{
		return Vector2f{ float(rand() % range - (range / 2)), float(rand() % range - (range / 2)) };
	}
	else
	{
		return Vector2f{ float(rand() % range) , float(rand() % range) };
	}
}

bool utils::GetVecEqual(const Vector2f& vec1, const Vector2f& vec2)
{
	return std::abs(vec1.x - vec2.x) <= 0.001f;
}

float utils::GetRadians(const float degree)
{
	return float(degree / 180.f * M_PI);
}

float utils::GetDegrees(const float randian)
{
	return float(randian / M_PI * 180.f);
}

float utils::GetAngle(const Vector2f& vec1, const Vector2f& vec2)
{
	return atan2f(GetCrossProduct(vec1, vec2), GetDotProduct(vec1, vec2));
}

float utils::GetAngle(const Vector2f& vec)
{
	return atan2f(vec.y, vec.x);
}

float utils::GetDotProduct(const Vector2f& vec1, const Vector2f& vec2)
{
	return vec1.x * vec2.x + vec1.y * vec2.y;
}

float utils::GetCrossProduct(const Vector2f& vec1, const Vector2f& vec2)
{
	return ((vec1.x * vec2.y) - (vec1.y * vec2.x));
}

float utils::GetVectorLenght(const Vector2f& vec)
{
	return sqrtf(powf(vec.x, 2.f) + powf(vec.y, 2.f));
}

std::string utils::VecToString(const Vector2f& vec)
{
	return std::string("[" + std::to_string(vec.x) + ", " + std::to_string(vec.y) + "]");
}

Vector2f utils::GetScaledVector(const Vector2f& vec, const float scalar)
{
	return Vector2f(vec.x * scalar, vec.y * scalar);
}

Vector2f utils::GetNormalizedVector(const Vector2f& vec)
{
	const float lenght{ utils::GetVectorLenght(vec) };
	return Vector2f(vec.x / lenght, vec.y / lenght);
}

Vector2f utils::GetAddVectors(const Vector2f& vec1, const Vector2f& vec2)
{
	return Vector2f(vec1.x + vec2.x, vec1.y + vec2.y);
}

Point2f utils::GetAddVectors(const Point2f& point, const Vector2f& vec)
{
	return Point2f(point.x + vec.x, point.y + vec.y);
}

Vector2f utils::GetSubtractVectors(const Vector2f& vec1, const Vector2f& vec2)
{
	return Vector2f(vec1.x - vec2.x, vec1.y - vec2.y);
}

Point2f utils::GetSubtractVectors(const Point2f& point, const Vector2f& vec)
{
	return Point2f(point.x - vec.x, point.y - vec.y);
}

void utils::DrawVector(const Vector2f& vec, const Point2f& startPos)
{
...
void utils::PrintArray(const int* pNumbers, size_t size)
{
...
void utils::PrintArray(const int* pNumbers, int startIdx, const int endIdx)
{
...
int utils::GetArrayIndex(int rowIdx, int colIdx, int nrCols)
{
	return rowIdx * nrCols + colIdx;
}

int utils::CountArrValue(const int* arr, const int arrSize, const int searchValue)
{
	int returnValue{};
...
int utils::MinArrElement(const int* arr, const int arrSize)
{
	int minValue{ arr[0] };
	for (int idx{}; idx < arrSize; ++idx)
	{
...
int utils::MaxArrElement(const int* arr, const int arrSize)
{
	int maxValue{ arr[0] };
	for (int idx{}; idx < arrSize; ++idx)
	{
...
void utils::SwapInArr(int* arr, const int idx1, const int idx2)
{
...
void utils::SwapInArr(float* arr, const int idx1, const int idx2)
{
...
void utils::SwapInArr(Point2f* arr, const int idx1, const int idx2)
{
...
void utils::SwapInArr(Texture** arr, const int idx1, const int idx2)
{
...
void utils::ShuffleArr(int* arr, const int arrSize, const int shuffleTimes)
{
...
void utils::ShuffleArr(float* arr, const int arrSize, const int shuffleTimes)
{
	for (int idx{}; idx < shuffleTimes; ++idx)
	{
		int a{ rand() % arrSize };
...
void utils::ShuffleArr(Point2f* arr, const int arrSize, const int shuffleTimes)
{
	for (int idx{}; idx < shuffleTimes; ++idx)
	{
		int a{ rand() % arrSize };
...
void utils::ShuffleArr(Texture* arr, const int arrSize, const int shuffleTimes)
{
	for (int idx{}; idx < shuffleTimes; ++idx)
	{
		int a{ rand() % arrSize };
...
void utils::BubbleSort(int* arr, const int arrSize)
{
...
void utils::PrintVectorContainer(std::vector<int>& vec)
{
...
void utils::SwapInVector(std::vector<int>& vec, const size_t idx1, const size_t idx2)
{
	int swapValue{ vec[idx1] };
	vec[idx1] = vec[idx2];
	vec[idx2] = swapValue;
}

void utils::SwapInVector(std::vector<float>& vec, const size_t idx1, const size_t idx2)
{
	float swapValue{ vec[idx1] };
	vec[idx1] = vec[idx2];
	vec[idx2] = swapValue;
}

void utils::SwapInVector(std::vector<Texture*>& vec, const size_t idx1, const size_t idx2)
{
	Texture* swapValue{ vec[idx1] };
	vec[idx1] = vec[idx2];
	vec[idx2] = swapValue;
...
void utils::ShuffleVector(std::vector<int>& vec, const int shuffleTimes)
{
	for (int idx{}; idx < shuffleTimes; ++idx)
	{
		const int a{ rand() % int(vec.size()) };
		const int b{ rand() % int(vec.size()) };
		utils::SwapInVector(vec, a, b);
	}
}

void utils::ShuffleVector(std::vector<float>& vec, const int shuffleTimes)
{
	for (int idx{}; idx < shuffleTimes; ++idx)
	{
		const int a{ rand() % int(vec.size()) };
...
void utils::ShuffleVector(std::vector<Texture*>& vec, const int shuffleTimes)
{
	for (int idx{}; idx < shuffleTimes; ++idx)
	{
		const int a{ rand() % int(vec.size()) };
...
void utils::PrintColor(const Color4f& color)
{
	std::cout << "r: " << color.r << ", g: " << color.g << ", b: " << color.b << ", alpha" << color.a << "\n";
}

Color4f utils::GetRandomColor(const float alpha)
{
	Color4f randColor{};
	randColor.r = float(rand() % 256) / 255.f;
	randColor.g = float(rand() % 256) / 255.f;
	randColor.b = float(rand() % 256) / 255.f;
	randColor.a = alpha;

	return randColor;
}

Color4f utils::GetRGBColor(int r, int g, int b)
{
	Color4f color{};
...
bool utils::GetClockTime(float& runtime, const float elapsedSec, const float maxTime)
{
	runtime += elapsedSec;
...
bool utils::AreFloatsEqual(float one, float two, float epsilon)
{
